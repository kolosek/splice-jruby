...
module SpliceBenchmarks
  ...
  def self.profile
    ...
    Benchmark.benchmark(CAPTION, 20, FORMAT) do |x|
      ...
      company = Company.create!(id: nil, name: 'Company');
      ...
      company = Company.create!(id: nil, name: 'Company');
      x.report('where: ') { RUN_CIRCLE.times {|i| Company.where(name: 'Company').first } }
      DatabaseCleaner.clean

      company = Company.create!(id: nil, name: 'Company');
      3.times { User.create!(company: company) }
      company2 = Company.create!(id: nil, name: 'Company2');
      ...
      x.report('includes: ') { RUN_CIRCLE.times {|i| Company.includes(:users).find(company.id) } }
      DatabaseCleaner.clean

      company = Company.create!(id: nil, name: 'Company1');
